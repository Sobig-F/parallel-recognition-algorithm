...
public:
    ...
    LBPH(const cv::Mat img_);
    // ~LBPH();

    /**
     * @brief Сравнение LBPH
     */
    static double Similarity(const LBPH& standart, const LBPH& tested);
private:
    /**
    ...
    cv::Mat _LBPCode_r1 = cv::Mat::zeros(cv::Size(_img.size()), CV_8UC1);
    cv::Mat _LBPCode_r2 = cv::Mat::zeros(cv::Size(_img.size()), CV_8UC1);
    cv::Mat _LBPCode_r3 = cv::Mat::zeros(cv::Size(_img.size()), CV_8UC1);
};
} // namespace LBPH::cpu
